...
const LeadsManager: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (user?.role === 'employee') {
      ...
      if (editingLead) {
        createPendingAction('update', 'leads', { ...editingLead, ...leadData }, editingLead, user);
      } else {
        const newLead: Lead = {
          id: Date.now().toString(),
          ...leadData,
        ...
        createPendingAction('create', 'leads', newLead, undefined, user);
      }
  ...
  const handleDelete = (lead: Lead) => {
    if (user?.role === 'employee') {
      if (window.confirm('Your delete request will be sent to admin for approval. Continue?')) {
        createPendingAction('delete', 'leads', lead, undefined, user);
          ...
          const importedLeads = lines.slice(1)
            ...
            .map(line => {
              ...
              headers.forEach((header, index) => {
                ...
                if (key === 'budget') {
                  lead[key] = values[index] ? parseFloat(values[index]) : 0;
                } else {
  ...
  // Table columns for list view (partial data as specified)
  const columns = [
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6">
      ...
      {/* Add/Edit Modal - Full Form Fields */}
      <Modal
        isOpen={showModal}
        onClose={() => {
          setShowModal(false);
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-gray-200">
            ...
            >
              {editingLead ? 'Update Lead' : 'Create Lead'} {user?.role === 'employee' && '(Pending Approval)'}
            </button>
          </div>
        </form>
      </Modal>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-900">{selectedLead.inquiryDate}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-900">{selectedLead.leadManagerName}</p>
              </div>
              <div>
